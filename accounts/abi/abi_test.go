...
func TestMethodSignature(t *testing.T) {
	...
	idexp := crypto.Blake256([]byte(exp))[:4]
...
func TestMultiPack(t *testing.T) {
	...
	sig := crypto.Blake256([]byte("bar(uint32,uint16)"))[:4]
...
func TestABI_MethodById(t *testing.T) {
	...
	if _, err = abi.MethodById(crypto.Blake256()); err == nil {
...
func TestABI_EventById(t *testing.T) {
	...
	for testnum, test := range tests {
		...
		topic := test.event
		topicID := crypto.Blake256Hash([]byte(topic))
		...
		unknowntopicID := crypto.Blake256Hash([]byte("unknownEvent"))
