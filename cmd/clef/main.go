...
func attestFile(ctx *cli.Context) error {
	...
	vaultLocation := filepath.Join(configDir, common.Bytes2Hex(crypto.Blake256([]byte("vault"), stretchedKey)[:10]))
	confKey := crypto.Blake256([]byte("config"), stretchedKey)
...
func setCredential(ctx *cli.Context) error {
	...
	configDir := ctx.String(configdirFlag.Name)
	vaultLocation := filepath.Join(configDir, common.Bytes2Hex(crypto.Blake256([]byte("vault"), stretchedKey)[:10]))
	pwkey := crypto.Blake256([]byte("credentials"), stretchedKey)
...
func signer(c *cli.Context) error {
	...
	if stretchedKey, err := readMasterKey(c, ui); err != nil {
		...
		pwkey := crypto.Blake256([]byte("credentials"), stretchedKey)
		jskey := crypto.Blake256([]byte("jsstorage"), stretchedKey)
		confkey := crypto.Blake256([]byte("config"), stretchedKey)
...
func readMasterKey(ctx *cli.Context, ui core.UIClientAPI) ([]byte, error) {
	...
	vaultLocation := filepath.Join(configDir, common.Bytes2Hex(crypto.Blake256([]byte("vault"), masterSeed)[:10]))
